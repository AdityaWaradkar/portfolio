  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl w-full relative z-10">
          ...
          <div className="flex flex-col gap-6 sm:gap-8">
            <div>
              ...
              <p className="leading-relaxed word-spacing text-gray-300/90 text-sm sm:text-base md:text-lg">
                I build backend systems in Go and full-stack applications with
                the MERN stack. Currently deepening my expertise in DevOps and
                cloud-native development. Passionate about clean architecture,
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-4">
                {["Pencil Sketching", "Photography"].map((hobby) => (
                  <div
                    key={hobby}
                    className={`${cardBaseClass} min-w-[110px] sm:min-w-[130px]`}
                  >
                    <a
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-4">
                {skills.map((skill) => (
                  <div
                    key={skill}
                    className={`${cardBaseClass} min-w-[80px] sm:min-w-[100px]`}
                  >
                    {skill}
                  </div>
                ))}
        ...
        {/* Certifications - Full Width Section Below Both Columns */}
        <div className="mt-12 sm:mt-16 space-y-3 sm:space-y-4">
          <h2 className="text-2xl sm:text-3xl font-bold text-white/90 text-center lg:text-left">
            Certifications
          </h2>
          <div className="flex flex-wrap gap-2 sm:gap-4 justify-center lg:justify-start">
            {certifications.map((cert) => (
              <div
                key={cert.name}
                className={`${cardBaseClass} min-w-[130px] sm:min-w-[150px]`}
              >
                {cert.link ? (
                  <a
                    href={cert.link}
                  ...
                  <span className="block opacity-60">{cert.name}</span>
                )}
              </div>
            ))}
          </div>
